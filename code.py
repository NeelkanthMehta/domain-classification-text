...
df['category'] = [label_race(row) for indx, row in df.iterrows()]

remove = ['food', 'recharge', 'support', 'reminders', 'nearby', 'movies', 'casual', 'other', 'travel']
...
X = tfidf.fit_transform(np.array(all_text))
...
# Fitting and prediction a Logistic Regression model
log_reg = LogisticRegression(random_state=0)
log_reg.fit(X_train, y_train)
y_pred = log_reg.predict(X_val)
log_accuracy = accuracy_score(y_val, y_pred)

# Fitting and prediction a Naive Bayes model
nb = MultinomialNB()
nb.fit(X_train, y_train)
y_pred = nb.predict(X_val)
nb_accuracy = accuracy_score(y_val, y_pred)

# Fitting and prediction a Support Vector Classifier model
...
maxindx = coherence_value_list.index(max(coherence_value_list))
opt_topic = topic_list[maxindx]
